...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=55219700096523:+55 21 97009-6523\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
const config = {
    XBOT: 'TOKYO TEAM BOT', 
    instagram: '@tekashizzzj', 
    nomer: 'wa.me/5511934396442 se mandar trava vai pro suporte',
    youtube: 'https://youtube.com/悪魔tokyoteam',
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `salve puto @${num.split('@')[0]}\ bem vindo/a ao grupo *${mdata.subject}* se fizer merda vai de ban preto`
				...
				teks = `Mete o pé filho da puta@${num.split('@')[0]} saiu pq quis`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ ESPERA MAN,TA COM PRESSA? FAZ VOCE MESMO,SEM FLOOD MAN PFVR❬❗❭',
				success: '️❬ ✔ ❭ TA AI PUTO 🖤',
				error: {
					stick: 'Executa o comando direito doente',
					Iv: 'Olha o link pra ver se ta certo,analfabeto.'
				},
				only: {
					group: '❬❗❭ ESSE COMANDO SO PODE SER EXECUTADO EM GRUPOS❬❗❭ ',
					ownerG: '❬❗❭ SO OS ADMS DA TEAM PODEM USAR ESTE COMANDO❬❗❭ ',
					ownerB: '❬❗❭  DENOVO MAN? SÓ OS ADMS DA TEAM PODEM USAR ESSE CMD DOENTE❬❗❭  ',
					admin: ' ❬ ⚠️ ❭ COMANDO PERMITIDO SOMENTE PARA ADMS ❬ ⚠️ ❭ ',
					Badmin: '❬❗❭O BOT PRECISA SER ADM PRA USAR ESSE CMD MULA ;3 '
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5591985700356@s.whatsapp.net"] 
...
case 'lofi':
				...
				case 'ytmp3':
					...
					teks = `â *Titulo* : ${anu.title}\nâ *Tamanho* : ${anu.filesize}\n\nEspera seu filho da puta, Audio Esta Sendo Enviado Novamente...`
                ...
                case 'reportarbug':
                     const pesan = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Quer escrever a biblia fdp? maximo 300 letras seu lixo', msgType.text, {quoted: mek})
                    ...
                    reply('PDP PARSA, BUG FOI REPORTADO, SE FOR MENTIRA VOU COMER O CU DA DUA FML🙄✌🏼')
                    break
					case 'marcar':
					members_id = []
				...
				case 'ytbuscar':
				case 'ytbaixa':
					if (args.length < 1) return reply('Ta faltando o link man??')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					...
					break
                case 'logo3d':
              	    if (args.length < 1) return reply('quer escrever o que?,coloca o texto dps do comando man!')
                    teks = `${body.slice(8)}`
			...
			case 'mapa': 
				...
				case 'audio':
					...
					if (args.length < 2) return client.sendMessage(from, 'qual txt vc quer que eu fale?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('escreveu a biblia foi? limite de 300 letras👺')
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							fs.unlinkSync(ranm)
							buff = fs.readFileSync(rano)
							if (err) return reply('SLA MAN TENTA DNV^_^')
